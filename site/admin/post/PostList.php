...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($acao === 'criar') {
        if ($postModel->criar($dados)) {
            $mensagem = "Objetos criado com sucesso!";
            $acao = 'listar';
        }
    } elseif ($acao === 'editar') {
        $id = $_GET['id'];
        if ($postModel->atualizar($id, $dados)) {
            $mensagem = "Objetos atualizado com sucesso!";
            $acao = 'listar';
        }
    }
}
...
if (isset($_GET['excluir'])) {
    if ($postModel->delete($_GET['excluir'])) {
        $mensagem = "Objetos excluído com sucesso!";
    }
}
...
?>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="?acao=criar" class="btn btn-primary">
            <i class="fas fa-plus"></i> Novo Objetos
        </a>
    </div>

    <form method="GET" class="mb-3">
        <div class="input-group">
            <input type="text" name="termo" class="form-control" placeholder="Buscar Objetos..." 
...
<?php elseif ($acao === 'criar' || $acao === 'editar'): ?>
    <h2><?php echo $acao === 'criar' ? 'Criar' : 'Editar'; ?> Objetos</h2>
